...
botones.forEach(boton => boton.addEventListener("click", async function (event) {
...
async function obtenerDatosDePokemons() {
    const fetchPromises = Array.from({ length: totalPokemons }, (_, i) => fetch(URL + (i + 1)).then(response => response.json()));
...
function mostrarPokemon(data) {
    ...
    link.href = `indexPokedex.html?pokemon=${encodeURIComponent(JSON.stringify(data))}`;
