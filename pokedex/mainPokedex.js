...
function ajaxSuccess(data) {
  ...
  textType.innerHTML = `${type[0]?.type?.name || "Unknown"} ${
    type[1]?.type?.name || ""
  }`;
  textMoves.innerHTML = `${moves[0]?.move?.name || ""} ${
    moves[1]?.move?.name || ""
  } ${moves[2]?.move?.name || ""} ${moves[3]?.move?.name || ""}`;
  textStats.innerHTML = `${stats[0]?.stat?.name || ""}:${
    stats[0]?.base_stat || ""
